...
class LineGraph{
    ...
    public function generateGraph($IMEI, $START_DATE , $END_DATE)
    {
...
// Query to get all the specific beer type each flowmeter connected to the arduino
        $headingQuery =
            "SELECT  PORT_NUMBER, BEER_NAME FROM `tbl_ard_fm` 
    JOIN tbl_beer_fm on tbl_ard_fm.FM_ID = tbl_beer_fm.FM_ID
...
// Query to get all pouring data for the specific arduino between the specified time
    ...
    VOLUME FROM tbl_log 
    WHERE ARD_IMEI_NUMBER = $IMEI AND LOG_DATE BETWEEN  $START_DATE AND $END_DATE
    ORDER BY PORT_NUMBER
";
